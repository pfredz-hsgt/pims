...
const CartPage = () => {
    ...
    const generatePDFManufact = (items) => {
        // 1. Initialize Landscape PDF
        const doc = new jsPDF({ orientation: 'landscape' });
        ...
        const externalItems = items.filter(i => i.inventory_items?.indent_source === 'MNF External');
        const internalItems = items.filter(i => i.inventory_items?.indent_source === 'MNF Internal');
        ...
        copies.forEach((copyLabel, copyIndex) => {
            ...
            doc.setFontSize(10); // Slightly smaller title to fit
            doc.setFont(undefined, 'bold');
            const title = `BORANG PERMOHONAN STOK UBAT (Manufact)`;
            doc.text(title, panelCenter, yPosition, { align: 'center' });
            yPosition += 5;

            // --- Data Preparation with Section Headers ---
            const tableData = [];
            ...
            // Section 1: EXTERNAL
            tableData.push([{ content: 'EKSTERNAL', colSpan: 6, styles: { halign: 'center', fontStyle: 'bold', fillColor: [240, 240, 240] } }]);
            if (externalItems.length > 0) {
                externalItems.forEach((item, idx) => tableData.push(mapItem(item, idx)));
            } else {
                tableData.push([{ content: '-', colSpan: 6, styles: { halign: 'center' } }]);
            }

            // Section 2: INTERNAL
            tableData.push([{ content: 'INTERNAL', colSpan: 6, styles: { halign: 'center', fontStyle: 'bold', fillColor: [240, 240, 240] } }]);
            if (internalItems.length > 0) {
                internalItems.forEach((item, idx) => tableData.push(mapItem(item, idx)));
            } else {
                tableData.push([{ content: '-', colSpan: 6, styles: { halign: 'center' } }]);
            }
            ...
            autoTable(doc, {
                ...
                head: [[
                    ...
                    { content: 'Catatan', styles: { halign: 'center' } },
                    { content: 'Lulus', styles: { halign: 'center' } },
                    { content: 'Catatan', styles: { halign: 'center' } },
                ]],
                body: tableData,
                theme: 'grid',
                ...
                margin: { left: panelLeft + 5, right: (pageWidth - panelRight) + 5 },
                columnStyles: {
                ...
                didParseCell: function (data) {
                    // If the row content is one of our headers, increase height
                    if (data.row.section === 'body' && (data.cell.raw.content === 'Manufacturing EXTERNAL' || data.cell.raw.content === 'Manufacturing INTERNAL')) {
    ...
    const processPDFExport = (mode) => {
        try {
            let exportCount = 0;
            const mnfSources = ['MNF External', 'MNF Internal'];
            const mnfItems = [];

            // 1. Check if we have any MNF sources selected and gather their items
            mnfSources.forEach(source => {
                if (selectedSources.includes(source) && groupedItems[source]?.length > 0) {
                    mnfItems.push(...groupedItems[source]);
                }
            });

            // 2. If we have MNF items, generate ONE combined specialized PDF
            if (mnfItems.length > 0) {
                const doc = generatePDFManufact(mnfItems);

                const timestamp = new Date().toISOString().split('T')[0];
                const filename = `Indent_MFG_Combined_${timestamp}.pdf`;
            ...
            // 3. Process remaining sources as usual
            Object.entries(groupedItems).forEach(([source, items]) => {
                // Skip if not selected OR no items
                if (items.length === 0 || !selectedSources.includes(source)) return;

                // Skip MNF sources as they are handled above
                if (mnfSources.includes(source)) return;
